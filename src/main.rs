...
use crates_io_api::{SyncClient, Error as CrateError};
...
fn main() {
    ...
    let first_alphabetic = crate_name.chars().next().map(char::is_alphabetic).unwrap_or(false);
    let not_empty = !crate_name.is_empty();
    let all_ascii_alphanum = crate_name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '-');
    if !(not_empty && all_ascii_alphanum && first_alphabetic && under_max_length) {
        eprintln!("error: crate name '{}' is not valid for crates.io", crate_name);
        std::process::exit(-1);
    }
    ...
    let exists = match check_crate_exists(crate_name) {
        Ok(e) => e,
        Err(e) => {
            eprintln!("error: failed to lookup information for crate '{}': {}", crate_name, e);
            std::process::exit(-1);
        }
    };
    ...
    if open_doc {
        match opener::open(format!("https://docs.rs/{}", crate_name)) {
            Err(e) => {
